...
/* ── Percentage labels inside doughnut ── */
const pctPlugin = {
  id: 'pctLabels',
  afterDraw(chart) {
...
const ORDER_COLORS  = ['#c4a45d', '#7ab3e0', '#c990e0', '#6ecfb2', '#e07070'];
const ORDER_LABELS  = ['Pending', 'Confirmed', 'Processing', 'Completed', 'Cancelled'];
const ORDER_DATA    = [
  parseInt(document.querySelector('[data-pending]')?.dataset.pending   || 0),
  0,
  0,
  parseInt(document.querySelector('[data-completed]')?.dataset.completed || 0),
  parseInt(document.querySelector('[data-cancelled]')?.dataset.cancelled || 0),
];
...
drawPie('order-chart', R_PAY_DATA,  PAY_LABELS,   PAY_COLORS);