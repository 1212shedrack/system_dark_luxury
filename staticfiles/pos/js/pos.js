...
function addToCart(id, name, price, quantity) {
  if (cart[id]) {
    if (cart[id].qty >= quantity) {
      flashMsg(`Only ${quantity} in stock`);
      return;
    }
    cart[id].qty += 1;
  } else {
...
function updateQty(id, delta) {
  ...
  if (newQty <= 0) {
    ...
    flashMsg(`Only ${cart[id].quantity} in stock`);
    return;
  } else {
    cart[id].qty = newQty;
  }
...
async function processSale() {
  ...
  try {
    ...
    if (data.success) {
      showOverlay(`Sale complete! Change: ${fmt(data.change)}`);
      setTimeout(() => {
        cart = {};
        renderCart();
        hideOverlay();
  ...
  if (!results.length) {
    ...
    box.innerHTML = results.map(p => `
      <div class="search-result-item"
           ...
           ${p.price}, ${p.quantity}); closeSearch()">
        <span class="sr-name">${p.name}</span>
        <span class="sr-price">${fmt(p.price)}</span>
        <span class="sr-qty">${p.quantity} left</span>
      </div>
    `).join('');
  }
...
function flashMsg(msg) {
  ...
  el.style.cssText = `
    ...
    padding:8px 18px;border-radius:8px;font-size:12px;z-index:999;
